...
class BankAccount {
    ...
    setTransactionMethod(transactionMethod: TransactionMethod) {
        this.transactionMethod = transactionMethod;
    }

    makeTransaction(amount: number) {
        if (!this.transactionMethod) {
            console.error("Transaction method not set.");
            return;
        }
...
const johnsAccount = new BankAccount(1000);

johnsAccount.setTransactionMethod(webApp);
johnsAccount.makeTransaction(100);  // Usa WebApp

johnsAccount.setTransactionMethod(mobileApp);
johnsAccount.makeTransaction(150);  // Usa MobileApp

johnsAccount.setTransactionMethod(atmMachine);
johnsAccount.makeTransaction(200);  // Usa ATM
